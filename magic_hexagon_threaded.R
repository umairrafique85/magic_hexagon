...
tillrow2 <- mclapply(po3list, getrow2)
...
tillrow2 <- mclapply(tillrow2, fix2list)
...
tillrow3 <- mclapply(tillrow2, function(lvl1){
  currelement <<- lvl1
...
tillrow3 <- mclapply(tillrow3, function(lvl1){
...
tillrow4 <- mclapply(tillrow3, function(lvl1){
  currelement1 <<- lvl1
  lvl1[[2]] <- lapply(lvl1[[2]], function(lvl2){
    currelement2 <<- lvl2
...
tillrow4 <- mclapply(tillrow4, function(lvl1){
...
tillrow5 <- mclapply(tillrow4, function(lvl1){
  currelement1 <<- lvl1
  lvl1[[2]] <- lapply(lvl1[[2]], function(lvl2){
    currelement2 <<- lvl2
    lvl2[[2]] <- lapply(lvl2[[2]], function(lvl3){
...
tillrow5 <- mclapply(tillrow5, function(lvl1){
  lvl1[[2]] <- lvl1[[2]][sapply(lvl1[[2]], function(x){length(x[[2]])>0})]
  lvl1
})
tillrow5 <- tillrow5[sapply(tillrow5, function(x){length(x[[2]])>0})]
...
final_neat <- mclapply(tillrow5, function(hex){
  row1 <- hex[[1]]
  row2 <- hex[[2]][[1]][[1]]
  row3 <- hex[[2]][[1]][[2]][[1]][[1]]
